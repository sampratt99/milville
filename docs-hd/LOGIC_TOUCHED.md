...
| 42 | index.html | The three `HDX` hook calls in the render loop (`frame`, `render`, `resize`) are wrapped in try/catch; a throw reports to `HDX.fail` and the frame falls back to `renderer.render`. Vanilla path (no `HDX`) unchanged. |
| 42 | hd-pre.js | Old School mode: the file returns before any class swap when `milville-hd-mode` is `off`, when WebGL2 is missing, or when the crash marker `milville-hd-boot` was left set. Reads `saveGame`/`player` only when switching modes, to save before the reload. |
